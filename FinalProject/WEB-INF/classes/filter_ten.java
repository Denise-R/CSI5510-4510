...
public class filter_ten extends HttpServlet 
{
    ...
    {        
		...
		query = "SELECT m.MovieID, m.Title, p.FirstName, p.LastName, p.PersonType FROM MOVIE m, PERSON p, CAST c WHERE m.MovieID = c.MovieID AND p.personID = c.personID AND p.PersonID = '" + p_id + "' AND (p.PersonType = 'Actor' OR p.PersonType = 'Actress')";
		
		out.println("<html><head><title>FinalProject</title>");	 
		out.println("</head><body>");
		
		out.print( "<br /><b><center><font color=\"RED\"><H2>Movies by Cast</H2></font>");
		...
		out.println("<tr BGCOLOR=\"#cccccc\">");
          ...
          out.println("<td align = \"justify\"><font face =\"times new roman\"  size=\"4pt\">Cast Type</td>");
        out.println("</tr>");
		try 
		{ 
            while(result.next()) 
